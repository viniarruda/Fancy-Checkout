...
class Checkout {
  ...
  submit() {
    ...
    this.cardNameBack.insertAdjacentHTML(
      "afterbegin",
    ...
    this.cardBack.addEventListener(
      "animationend",
      () => {
        ...
        this.willDisappear.addEventListener(
          "animationend",
          () => {
            this.willDisappearAll.forEach(item => item.remove());
            this.total.style.cssText =
              "padding:15px; border:2px solid rgba(255,255,255,0.7); border-radius:4px;";
